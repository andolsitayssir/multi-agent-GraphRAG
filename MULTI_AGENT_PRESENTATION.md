...
The `GraphRAG` class ([graph.py](file:///c:/Users/lando/multi-agent-genAI/app/graph.py)) is the core component that handles all interactions with the Neo4j knowledge graph database.
...
The backend is powered by **FastAPI** ([api.py](file:///c:/Users/lando/multi-agent-genAI/app/api.py)).
...
We implemented an automated evaluation suite ([evaluate.py](file:///c:/Users/lando/multi-agent-genAI/app/evaluate.py)) to test the system's accuracy and performance.
